...
public class EvNetworkRoutingProviderRwa implements Provider<RoutingModule> {
	private static final Logger log = Logger.getLogger(EvNetworkRoutingProviderRwa.class);
	 ...
	 * @param mode
	 */
	public EvNetworkRoutingProviderRwa(String mode) {
		this(mode, mode);
	}
	 ...
	 * @param routingMode
	 */
	public EvNetworkRoutingProviderRwa(String mode, String routingMode) {
		this.mode = mode;
	...
	public RoutingModule get() {
		...
		if (!plansCalcRouteConfigGroup.getAccessEgressType().equals(AccessEgressType.none)) {
			...
			return new EvNetworkRoutingModuleRwa(mode, filteredNetwork,
					DefaultRoutingModules.createPureNetworkRouter(mode, populationFactory, filteredNetwork, routeAlgo),
					electricFleetSpecification, chargingInfrastructureSpecification, EvConfigGroup.get(config));
		}
	}
}
